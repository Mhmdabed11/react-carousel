...
export default function Carousel() {
  ...
  const [activeSlides, setActiveSlides] = React.useState(6);
  const [width, setWidth] = React.useState(window ? window.innerWidth : 0);
  const [currentIndex, setCurrentIndex] = React.useState(0);
  const [slidesToMove, setSlidesToMove] = React.useState(0);
  ...
  React.useEffect(() => {
    if (width > 1200) {
      if (activeSlides !== 6) setActiveSlides(6);
    }
  }, [width]);

  const remainingSlides = movies.length - (currentIndex + activeSlides);
  ...
  const styles =
    hasMovedOnce && !isSliding
      ...
      : !hasMovedOnce && isSliding && direction === 1
      ? { transform: "translateX(-100%)", transition: "all 400ms ease" }
      : isSliding && direction === -1
      ? { transform: "translateX(0%)", transition: "all 400ms ease" }
  ...
  const handleNext = () => {
    ...
    let remainingSlides = movies.length - (currentIndex + activeSlides);
    if (remainingSlides >= activeSlides) {
      setSlidesToMove(activeSlides);
      setCurrentIndex(currentIndex => currentIndex + activeSlides);
    } else {
      setSlidesToMove(remainingSlides);
      setCurrentIndex(currentIndex => currentIndex + remainingSlides);
    }
  ...
  return (
    <div className="slider">
      ...
      <button disabled={!hasMovedOnce} onClick={handlePrevious}>
        previous
      </button>
